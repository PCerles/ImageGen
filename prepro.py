...
def prepro_captions(path):
  captions = [] 
  for filename in os.listdir(path):
	img = {}
	read_file = open(path + filename, 'r')
	capt = read_file.read()
	img['file'] = filename[:-4]
	txt = str(capt).lower().translate(None, string.punctuation).strip().split()
...
def encode_captions(imgs, params, wtoi):
  ...
  for i,img in enumerate(imgs):
	...
	np.savetxt(os.path.join(path, img['file'] + '.csv'), Li, fmt='%i', delimiter='\n')
	#with open(os.path.join(path, img['file']+'.csv'), 'wb') as csvfile:
	#	spamwriter = csv.writer(csvfile, delimiter='\n',
        #                    quotechar='|', quoting=csv.QUOTE_MINIMAL)
	#	spamwriter.writerow(list(Li))
...
def main(params):
  path = "captionsSmall3/"
  imgs = prepro_captions(path)     
  # create the vocab
  vocab = build_vocab(imgs, params)
  itow = {i+1:w for i,w in enumerate(vocab)} # a 1-indexed vocab translation table
  wtoi =json.load(open('output.json'))['w_to_i']
